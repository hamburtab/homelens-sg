...
OUTPUT_FILE = PROCESSED_DIR / "live_sale_listings.csv"
GEOCODE_CACHE_FILE = PROCESSED_DIR / "live_sale_listing_geocode_cache.json"
...
def make_listing_id(source_url: str, address: str, price: int | None) -> str:
    key = f"{source_url}|{address}|{price}"
    return hashlib.sha1(key.encode("utf-8")).hexdigest()[:12]


def parse_price(text: str) -> int | None:
    match = re.search(r"S\$\s*([\d,]+)(?!\s*(?:psf|/mo))", text, flags=re.I)
...
def parse_price_per_sqft(text: str) -> float | None:
    match = re.search(r"S\$\s*([\d,.]+)\s*psf", text, flags=re.I)
...
def parse_bed_bath(text: str) -> tuple[int | None, int | None]:
    match = re.search(
        r"\b(?P<bedrooms>\d+)\s+(?P<bathrooms>\d+)\s+[\d,]+\s*sqft\b",
        text,
        flags=re.I,
    )
    ...
    return int(match.group("bedrooms")), int(match.group("bathrooms"))
...
def parse_flat_type(text: str) -> str | None:
    patterns = [
        r"\b\d+\s*I\s+HDB\b",
        r"\b\d+\s*NG\s+HDB\b",
        r"\b\d+\s*ROOM\s+HDB\b",
        r"\bExecutive\s+HDB\b",
        r"\bMaisonette\b",
    ]
    ...
    return "HDB Flat" if "HDB Flat" in text else None


def parse_address(text: str) -> str | None:
    match = re.search(
        r"(?:S\$\s*[\d,]+)(?:\s+S\$\s*[\d,.]+\s*psf)?\s+(.+?)\s+\1\b",
        text,
        flags=re.I,
    )
    ...
    match = re.search(
        r"(?:S\$\s*[\d,]+)(?:\s+S\$\s*[\d,.]+\s*psf)?\s+(.+?)\s+(?:\d+\s+\d+\s+[\d,]+\s*sqft|HDB Flat|Listed on)",
        text,
        flags=re.I,
    )
...
def parse_title(text: str, price: int | None, address: str | None) -> str | None:
    if price is None:
        return None
...
def listing_links_from_page(html: str, base_url: str) -> list[tuple[str, str]]:
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if "S$" not in text or "/mo" in text:
            continue
...
def parse_propertyguru_listing(
    ...
    bedrooms, bathrooms = parse_bed_bath(text)
    nearest_mrt_name, nearest_mrt_distance_m = parse_mrt(text)

    return {
        "listing_id": make_listing_id(source_url, address, price),
        "source": "PropertyGuru",
        "source_listing_reference": parse_listing_reference(source_url),
        "title": parse_title(text, price, address),
        "price": price,
        "address": address,
        "property_type": "HDB Flat" if "HDB" in text else pd.NA,
        "flat_type": parse_flat_type(text),
        "bedrooms": bedrooms,
        "bathrooms": bathrooms,
        "floor_area_sqft": parse_area(text),
        "price_per_sqft": parse_price_per_sqft(text),
...
def scrape_propertyguru_page(
    ...
    for source_url, text in listing_links_from_page(html, page_url):
        listing = parse_propertyguru_listing(source_url, text)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape public live HDB sale listings and geocode them.",
    )
    parser.add_argument(
        "--start-url",
        default=DEFAULT_START_URL,
        help="PropertyGuru HDB sale search URL to start from.",
    )
    parser.add_argument(
        "--pages",
        type=int,
